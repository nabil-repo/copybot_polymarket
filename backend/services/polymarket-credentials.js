...
const ENCRYPTION_KEY_HEX = process.env.ENCRYPTION_KEY||'eb2f14903e4fb5728cc3d362ab95157d97685571c4174f23f93ae91a52ad236f';
...
export async function storePolymarketCredentials(userId, apiKey, apiSecret) {
  try {
    ...
    await run(
      ...
      [
        userId,
        JSON.stringify({ encrypted: encryptedKey.encrypted, authTag: encryptedKey.authTag }),
        JSON.stringify({ encrypted: encryptedSecret.encrypted, authTag: encryptedSecret.authTag }),
        encryptedKey.iv
      ]
    );
    
    console.log(`✅ Stored encrypted Polymarket credentials for user ${userId}`);
    return true;
  } catch (error) {
    console.error(`❌ Failed to store credentials for user ${userId}:`, error.message);
    return false;
  }
}
...
export async function getPolymarketCredentials(userId) {
  try {
    ...
    const apiSecret = decrypt(encryptedSecret.encrypted, iv, encryptedSecret.authTag);
    
    return { apiKey, apiSecret };
  } catch (error) {
    console.error(`❌ Failed to retrieve credentials for user ${userId}:`, error.message);
    return null;
  }
}
...
export async function deletePolymarketCredentials(userId) {
  try {
    ...
    console.error(`❌ Failed to delete credentials for user ${userId}:`, error.message);
    return false;
  }
}
