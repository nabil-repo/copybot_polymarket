...
userRouter.get('/profile', authMiddleware, async (req, res) => {
  const user = await get('SELECT id, email, execution_wallet FROM users WHERE id = ?', [req.user.id]);
...
userRouter.put('/execution-wallet', authMiddleware, async (req, res) => {
  ...
  try {
    await run('UPDATE users SET execution_wallet = ? WHERE id = ?', [wallet.toLowerCase(), req.user.id]);
...
userRouter.get('/wallets', authMiddleware, async (req, res) => {
  const rows = await all('SELECT wallet FROM user_wallets WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
...
userRouter.post('/wallets', authMiddleware, async (req, res) => {
  ...
  if (!wallet) return res.status(400).json({ error: 'missing_wallet' });
  try {
    await run('INSERT OR IGNORE INTO user_wallets (user_id, wallet) VALUES (?, ?)', [req.user.id, wallet.toLowerCase()]);
    res.json({ ok: true });
  } catch (err) {
...
userRouter.get('/bot-status', authMiddleware, async (req, res) => {
  try {
    const statusRow = await get('SELECT status, started_at, stopped_at FROM bot_status WHERE user_id = ?', [req.user.id]);
    if (!statusRow) {
...
userRouter.post('/bot-start', authMiddleware, async (req, res) => {
  try {
    ...
    await run(`INSERT INTO bot_status (user_id, status, started_at, stopped_at) VALUES (?, 'running', CURRENT_TIMESTAMP, NULL)
      ON CONFLICT(user_id) DO UPDATE SET status='running', started_at=CURRENT_TIMESTAMP, stopped_at=NULL`, [req.user.id]);
...
userRouter.post('/bot-stop', authMiddleware, async (req, res) => {
  try {
    ...
    await run(`INSERT INTO bot_status (user_id, status, started_at, stopped_at) VALUES (?, 'stopped', NULL, CURRENT_TIMESTAMP)
      ON CONFLICT(user_id) DO UPDATE SET status='stopped', stopped_at=CURRENT_TIMESTAMP`, [req.user.id]);
